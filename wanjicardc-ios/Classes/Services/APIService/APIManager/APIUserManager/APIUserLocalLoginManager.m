...
@implementation APIUserLocalLoginManager
...
{
    return self.loginname.length > 0 && self.password.length > 0;
}
...
{
    NSString * registrationID = [PushManager registrationID]?:@"no regid";

#if TARGET_IPHONE_SIMULATOR
    registrationID= @"05157fef936";
#endif
    
    return @{@"phone" : self.loginname,
             @"verificationCode" : self.password,
             @"userPushID" : registrationID,
             @"appChannel" : @"appStore"};
