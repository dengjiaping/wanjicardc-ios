...
@implementation APIBuyProductManager
#pragma mark - life cycle
- (instancetype)init
{
    self = [super init];
    if (self) {
//        self.channelType = DefautType;
//        self.proNum = 1;            //充值默认是1
        self.channelType = @"pingapp";
        self.specialCode = @"";
...
{
    return (self.proID.length > 0) && (self.merId.length > 0);
}
...
- (NSDictionary *)paramsForApi:(APIBaseManager *)manager
{
    NSMutableDictionary * dic = [NSMutableDictionary dictionaryWithObjectsAndKeys:
                                 [NSString stringWithFormat:@"%.2f",self.cardOrderAmount],
                                                                    @"cardOrderAmount",
                                 [NSString stringWithFormat:@"%@",@(self.orderType)],
                                                                    @"orderType",
                                 self.activityId?:@"",              @"activityId",
                                 self.isLimitForSale?:@"",          @"isLimitForSale",
                                 self.proID,                        @"merchantCardId",
                                 self.merId,                        @"branchMerchId",
                                 self.specialCode,                  @"userCouponCode",
                                 self.channelType,                  @"channelId",
                                 //                                 [NSString stringWithFormat:@"%ld",(long)self.proNum],
                                 //                                                                    @"cardOrderNumber",
                                 nil];
    
   
//    if (self.cashier.length > 0) {
//        [dic addEntriesFromDictionary:@{@"cashier":self.cashier}];
//    }
