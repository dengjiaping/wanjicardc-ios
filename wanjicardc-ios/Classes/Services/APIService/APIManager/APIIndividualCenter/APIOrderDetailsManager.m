...
@implementation APIOrderDetailsManager
...
{
    if (self.orderType == OrderTypeBaoZiCharge || self.orderType == OrderTypeElectronicCard) {
        
        return @{@"orderNo":self.ordersID ? : @"",
                 @"orderType":NumberToString(self.orderType) ? : @""};
        
    } else {
        
        return @{@"cardOrderNo":self.ordersID ? : @""};
    }
}
...
- (NSString *)methodName
{
    if (self.orderType == OrderTypeBaoZiCharge || self.orderType == OrderTypeElectronicCard) {
        ...
        return @"/userCenter/ordersDetail";
    } else {
        
        return @"/userCardOrder/get";
    }
}
