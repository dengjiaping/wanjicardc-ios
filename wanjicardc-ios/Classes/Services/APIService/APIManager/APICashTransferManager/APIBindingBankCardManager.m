...
@implementation APIBindingBankCardManager
...
{
    return self.name.length > 0 && self.bankCardNum.length > 0 && self.phoneNum.length > 0 && self.identityCard.length > 0;
...
{
    return @{@"trueName" :self.name,
             @"cardNo" :self.bankCardNum,
             @"phone" :self.phoneNum,
             @"messageCode" :self.verfiCodeNum ?:@"",
             @"idNo" :self.identityCard
             };
}
...
{
    return kAPIServiceCashTransfer;
}
