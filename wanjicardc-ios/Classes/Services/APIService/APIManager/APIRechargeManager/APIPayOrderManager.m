...
NSString *const kChargeTypeAlipay       =   @"40";
NSString *const kChargeTypeYiLianPayWap    =   @"55";


@implementation APIPayOrderManager
...
{
    return self.proID.length > 0  &&  self.orderNum.length > 0;
}
...
//    NSString  * curUUID = [[currentDevice identifierForVendor] UUIDString];
    
    return [NSDictionary dictionaryWithObjectsAndKeys:
            self.orderNum,@"orderno",
            self.proID,@"pcid",
            self.chargeType,@"payChannel",
            [WJGlobalVariable sharedInstance].currentID,@"imei",
            nil];
}
