...
namespace com.payeco
{
    ...
    public class TransactionClient
    {
                ...
                String priKey, String pubKey, String payecoUrl, Xml retXml)
        {
            ...
            LogPrint.println("PublicKey=" + priKey);
         ...
         * @param sign			：签名数据
         * @param pubKey		：易联签名验证公钥
         * @return              : true:验证通过； fase：验证失败
         * @throws Exception    :
         */
        public static bool bCheckNotifySign(String version, String merchantId, String merchOrderId,
                String amount, String extData, String orderId,
                ...
                String pubKey)
        {
            ...
            if (extData != null)
            {
                ...
                extData = Base64.DecodingString(extData, ConstantsClient.PAYECO_DATA_ENCODE);
                LogPrint.println("extData=" + extData); // 日志输出，检查转码是否正确
            }
            ...
            // 验证签名
            sign = sign.Replace(" ", "+");
            bool b = Signatory.verify(pubKey, data, sign, ConstantsClient.PAYECO_DATA_ENCODE);
            LogPrint.println("PublicKey=" + pubKey);
            LogPrint.println("data=" + data);
            LogPrint.println("Sign=" + sign);
            LogPrint.println("验证结果=" + b);
            return b;
        }
