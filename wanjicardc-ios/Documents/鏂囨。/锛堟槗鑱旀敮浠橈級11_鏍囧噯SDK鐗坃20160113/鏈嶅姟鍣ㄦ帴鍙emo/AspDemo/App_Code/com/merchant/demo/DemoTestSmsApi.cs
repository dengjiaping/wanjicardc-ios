...
namespace com.merchant.demo
{
    ...
    {
        private static String MerchOrderId_smId = "432255464"; //短信凭证号，发送和验证时要保持一致
        ...
        public static void MerchantOrderSendSmCodeTest() {
		    ...
		    // 调用下单接口
		    System.Console.WriteLine("-------【短信+API接口版本】发送短信验证码接口测试-------------------------");
		    try {
			    Xml retXml = new Xml();
                ...
                if (!"0000".Equals(ret))
                {
				    System.Console.WriteLine("【短信+API接口版本】发送短信验证码接口测试失败！：retCode="+ret+"; msg="+retXml.getRetMsg());
				    return;
			    }
		    } catch (Exception e) {
                System.Console.WriteLine("【短信+API接口版本】发送短信验证码接口测试失败！：e=" + e.Message);
			    return;
		    }
		    System.Console.WriteLine("【短信+API接口版本】发送短信验证码接口测试----ok");
        ...
        public static void MerchantOrderPayByAccTest(String smCodePay) {
            ...
            String miscData = "13922897656|0||张三|440121197511140912|62220040001154868428||PAYECO20151119454546||2|";  //互联网金融
		    ...
		    // 调用下单接口
		    System.Console.WriteLine("-------【短信+API接口版本】无磁无密交易接口测试-------------------------");
		    try {
			    Xml retXml = new Xml();
			    //接口参数请参考TransactionClient的参数说明
                ...
                if (!"0000".Equals(ret))
                {
				    System.Console.WriteLine("【短信+API接口版本】无磁无密交易接口测试失败！：retCode="+ret+"; msg="+retXml.getRetMsg());
				    return;
			    }
		    } catch (Exception e) {
                System.Console.WriteLine("【短信+API接口版本】无磁无密交易接口测试失败！：e=" + e.Message);
			    return;
		    }
		   System.Console.WriteLine("【短信+API接口版本】无磁无密交易接口测试----ok");
           System.Console.WriteLine("------------------------------------------------");
	    }
