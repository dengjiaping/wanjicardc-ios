...
namespace com.merchant.demo
{
    ...
    public class DemoTestSmsH5
    {
        ...
        public static void MerchantOrderSmsH5PayTest() {
            ...
            String miscData = "13922897656|0||张三|440121197511140912|62220040001154868428||PAYECO2015111944356||2|";  //互联网金融
		    ...
		    // 调用下单接口
		     System.Console.WriteLine("-------【短信+H5页面版本】商户下单接口测试-------------------------");
		    try {
			    Xml retXml = new Xml();
                ...
                if (!"0000".Equals(ret))
                {
				     System.Console.WriteLine("【短信+H5页面版本】商户下单接口测试失败！：retCode="+ret+"; msg="+retXml.getRetMsg());
				    return;
			    }
		    } catch (Exception e) {
                System.Console.WriteLine("【短信+H5页面版本】商户下单接口测试失败！：e=" + e.Message);
			    return;
		    }
		     System.Console.WriteLine("【短信+H5页面版本】商户下单接口测试----ok");
        ...
        public static void OrderSmsSendAgainTest(String orderId) {
		    ...
		    // 调用查询接口
		    System.Console.WriteLine("-------【短信+H5页面版本】重新发送支付短信接口测试-------------------------");
		    try {
			    Xml retXml = new Xml();
			    //接口参数请参考TransactionClient的参数说明
                ...
                if (!"0000".Equals(ret))
                {
				    System.Console.WriteLine("【短信+H5页面版本】重新发送支付短信接口测试失败！：retCode="+ret+"; msg="+retXml.getRetMsg());
				    return;
			    }
		    } catch (Exception e) {
                System.Console.WriteLine("【短信+H5页面版本】重新发送支付短信接口测试失败！：e=" + e.Message);
			    return;
		    }
		    System.Console.WriteLine("【短信+H5页面版本】重新发送支付短信接口测试----ok");
            System.Console.WriteLine("------------------------------------------------");
	    }
        ...
        public static void Test()
        {
            ...
            // 重发短信接口测试
            String orderId = "1449479575838";//填写已经成功下单的商户订单号；需要间隔60秒后才能重发
